...
auth-backend supports deployment to [Cloudflare Workers](https://workers.cloudflare.com) via the `wrangler` CLI. The entry point for Workers is `src/index.ts`. Workers run on Cloudflare's edge network and do not use a persistent Node.js process — environment variables are supplied as **secrets** rather than a `.env` file.
...
<Steps>
  ...
  <Step title="Configure secrets">
    ...
    If you use OAuth providers, add these optional secrets as well:
