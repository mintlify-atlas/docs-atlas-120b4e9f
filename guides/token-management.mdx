...
Both tokens are stored in **HTTP-only cookies** after email/password login and OAuth authentication. The one exception is **OTP login** — `POST /verify-otp` returns tokens directly in the response body without setting cookies, leaving storage to the client.
